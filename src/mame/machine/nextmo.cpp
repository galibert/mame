...
const uint32_t nextmo_device::t_rem[256] = {
...
const uint8_t nextmo_device::t_exp[768] = {
...
const uint8_t nextmo_device::t_log[256] = {
...
uint32_t nextmo_device::ecc_block(const uint8_t *s, int ss)
{
...
void nextmo_device::compute_one_ecc(int off, int step)
{
...
void nextmo_device::compute_ecc()
{
	...
	for(int i=0; i<32; i++)
		compute_one_ecc(i, 36);
	for(int i=0; i<36; i++)
		compute_one_ecc(36*i, 1);
}

int nextmo_device::fix_one_ecc(int off, int step)
{
	uint32_t ecc = ecc_block(sector+off, step);
	uint32_t ref_ecc =
...
{
	int ecount = 0;
	bool errors_left, errors_fixed;
	do {
		errors_left = false;
		errors_fixed = false;
		for(int i=0; i<32; i++) {
			int ec = fix_one_ecc(i, 36);
			if(ec == -1)
				errors_left = true;
			else if(ec) {
				ecount += ec;
	...
	} while(errors_left && errors_fixed);
