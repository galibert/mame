...
bool address_map_entry::unitmask_is_appropriate(UINT8 width, UINT64 unitmask, std::string string)
{
	// if no mask, this must match the default width of the map
	if (unitmask == 0)
	{
		if (m_map.m_databits != width)
			throw emu_fatalerror("Handler %s is a %d-bit handler but was specified in a %d-bit address map", string.c_str(), width, m_map.m_databits);
		return true;
	}

	// if we have a mask, we must be smaller than the default width of the map
	if (m_map.m_databits < width)
		throw emu_fatalerror("Handler %s is a %d-bit handler and is too wide to be used in a %d-bit address map", string.c_str(), width, m_map.m_databits);
	...
	while (singlemask != 0)
	{
		if ((unitmask & singlemask) != 0 && (unitmask & singlemask) != singlemask)
			throw emu_fatalerror("Handler %s specified a mask of %08X%08X; needs to be in even chunks of %X", string.c_str(), (UINT32)(unitmask >> 32), (UINT32)unitmask, basemask);
		singlemask <<= width;
	}
	return true;
}
	...
	for (address_map_entry *entry = m_entrylist.first(); entry != nullptr; entry = entry->next())
	{
		...
		if (!detected_overlap)
		{
			for (address_map_entry *scan = m_entrylist.first(); scan != entry; scan = scan->next())
				...
				{
					osd_printf_warning("%s space has overlapping memory (%X-%X,%d,%d) vs (%X-%X,%d,%d)\n", spaceconfig.m_name.c_str(), entry->m_addrstart, entry->m_addrend, entry->m_read.m_type, entry->m_write.m_type, scan->m_addrstart, scan->m_addrend, scan->m_read.m_type, scan->m_write.m_type);
		...
		if (!entry->m_region.empty() && entry->m_share.empty())
		{
				...
				for (const rom_entry *romp = rom_first_region(*dev); romp != nullptr && !found; romp = rom_next_region(romp))
				{
					if (rom_region_name(*dev, romp) == entry_region)
					{
						...
						if (entry->m_rgnoffs + (byteend - bytestart + 1) > length)
							osd_printf_error("%s space memory map entry %X-%X extends beyond region '%s' size (%X)\n", spaceconfig.m_name.c_str(), entry->m_addrstart, entry->m_addrend, entry->m_region.c_str(), length);
						found = true;
					}
				}
			...
			// error if not found
			if (!found)
				osd_printf_error("%s space memory map entry %X-%X references non-existant region '%s'\n", spaceconfig.m_name.c_str(), entry->m_addrstart, entry->m_addrend, entry->m_region.c_str());
		}
...
//      if ((entry->m_read.m_type == AMH_PORT && !entry->m_read.m_tag.empty() && portlist.find(entry->m_read.m_tag) == NULL) ||
//          (entry->m_write.m_type == AMH_PORT && !entry->m_write.m_tag.empty() && portlist.find(entry->m_write.m_tag) == NULL))
//          osd_printf_error("%s space memory map entry references nonexistant port tag '%s'\n", spaceconfig.m_name.c_str(), entry->m_read.m_tag.c_str());
