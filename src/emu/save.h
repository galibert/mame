...
public:
	...
	std::string indexed_item(int index, void *&base, UINT32 &valsize, UINT32 &valcount) const;
	...
	template<typename _ItemType>
	void save_item(_ItemType &value, std::string valname, int index = 0) { save_item(nullptr, "global", nullptr, index, value, valname); }
	template<typename _ItemType>
	void save_pointer(_ItemType *value, std::string valname, UINT32 count, int index = 0) { save_pointer(nullptr, "global", nullptr, index, value, valname, count); }

	// file processing
	static save_error check_file(running_machine &machine, emu_file &file, std::string gamename, void (CLIB_DECL *errormsg)(std::string fmt, ...));
...
private:
	...
	static save_error validate_header(const UINT8 *header, std::string gamename, UINT32 signature, void (CLIB_DECL *errormsg)(const char *fmt, ...), const char *error_prefix);
...
{
	save_memory(device, module, tag, index, name + ".attoseconds", &value.m_attoseconds, sizeof(value.m_attoseconds));
	save_memory(device, module, tag, index, name + ".seconds", &value.m_seconds, sizeof(value.m_seconds));
}
