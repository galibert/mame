...
std::string device_t::subtag(std::string tag) const
{
	std::string result;
	...
	int caret;
	while ((caret = tag.find('^')) != std::string::npos)
	{
		// copy everything up to there
		result += tag.substr(0, caret);
		tag.erase(0, caret+1);
...
device_interface::device_interface(device_t &device, std::string type)
