...
iwmfdc_device::iwmfdc_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock, uint32_t q3_clock) :
...
u64 iwmfdc_device::q3_to_fclk(u64 cycles) const
{
	return u64(m_q3_fclk_ratio * double(cycles) + 0.5);
}

u64 iwmfdc_device::fclk_to_q3(u64 cycles) const
{
...
void iwmfdc_device::device_timer(emu_timer &, device_timer_id, int, void *)
{
...
uint8_t iwmfdc_device::read(offs_t offset)
{
...
void iwmfdc_device::flush_write()
{
...
u8 iwmfdc_device::control(int offset, u8 data)
{
	sync();
	u8 changed = m_control;
	if(offset & 1)
	...
	if(changed & 0x20) {
		if(m_floppy)
			m_floppy->mon_w(true);
		m_floppy = m_control & 0x20 ?
			m_floppy1 ? m_floppy1->get_device() : nullptr :
			m_floppy0 ? m_floppy0->get_device() : nullptr;

		if(m_floppy)
			m_floppy->mon_w(m_active ? false : true);
	}
	...
	if(changed & 0x0f && m_floppy && m_active)
		m_floppy->seek_phase_w(m_control & 0xf);
...
void iwmfdc_device::mode_w(u8 data)
{
	m_mode = data;
...
void iwmfdc_device::data_w(u8 data)
{
	m_data = data;
...
u64 iwmfdc_device::time_to_cycles(const attotime &tm) const
{
	return tm.as_ticks(clock());
}

attotime iwmfdc_device::cycles_to_time(u64 cycles) const
{
	return attotime::from_ticks(cycles, clock());
}

bool iwmfdc_device::is_sync() const
{
	return m_mode & 0x02 ? false : true;
}

u64 iwmfdc_device::half_window_size() const
{
	switch(m_mode & 0x18) {
	case 0x00: return 14;
	case 0x08: return  7;
...
u64 iwmfdc_device::window_size() const
{
	switch(m_mode & 0x18) {
	case 0x00: return 28;
	case 0x08: return 14;
...
u64 iwmfdc_device::read_register_update_delay() const
{
	return m_mode & 0x08 ? 4 : 8;
}

u64 iwmfdc_device::write_sync_half_window_size() const
{
	...
	case MODE_READ: {
		...
		while(next_sync > m_last_sync) {
			if(next_flux_change <= m_last_sync) {
				attotime flux = m_floppy->get_next_transition(cycles_to_time(m_last_sync+1));
