...
void DeviceInformationWindow::fill_device_information()
{
	...
	if(intf) {
		...
		while(intf) {
			gl1->addWidget(new QLabel(QString(intf->interface_type().c_str()), primaryFrame), cpos, 1);
			cpos++;
	...
	if(m_device->interface(d_memory)) {
		...
		for(address_spacenum i=AS_0; i<ADDRESS_SPACES; i++)
			if(d_memory->has_space(i)) {
				...
				hb->addStretch();
				hb->addWidget(new QLabel(d_memory->space_config(i)->name().c_str()));
				vb->addWidget(ff);
			}
		vLayout->addWidget(f);
	}
...
std::string DeviceInformationWindow::device_tag() const
{
	return m_device->tag();
}
